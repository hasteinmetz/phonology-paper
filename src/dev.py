...
class Seq2Seq(nn.Module):  # Combine encoder and decoder into sequence-to-sequence model
        ...
        with warnings.catch_warnings():
            ...
            for e in tqdm(range(n_epochs)):
                ...
                for i, batch in enumerate(training_data.data_iter):
                    ...
                    target = torch.cat((target_la, target_tb, target_tc), axis=1)
    ...
    def evaluate_word(self, training_data, word, show_target=True):  # Evaluate the model's performance on a single word
        ...
        with torch.no_grad():
            ...
            target_tb = trial[2]
            target_tc = trial[3]
            target = torch.cat((target_la, target_tb, target_tc), 1)
        ...
        predicted_tc = training_data.linear_tr_tc.linear_transform_back(predicted[:, 2])
        ...
        figure_outputs, (tc_plot) = plt.subplots(1)
